...
class HelixOneClient:
    ...
    def _make_request(
        self, method: str, endpoint: str, data: dict | None = None, require_auth: bool = False
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            raise HelixOneAPIError(
                f"Timeout: le serveur ne répond pas après {self.timeout}s"
            ) from e
        except requests.exceptions.RequestException as e:
            raise HelixOneAPIError(f"Erreur réseau: {str(e)}") from e
    ...
    def is_license_valid(self) -> bool:
        ...
        try:
            ...
            return license["status"] == "active" and license["days_remaining"] > 0
        except Exception:
            return False
...
if __name__ == "__main__":
    ...
    try:
        result = client.register(
            email=test_email, password=demo_password, first_name="Demo", last_name="User"
        )
        print(f"✅ Compte créé: {result['user']['email']}")
